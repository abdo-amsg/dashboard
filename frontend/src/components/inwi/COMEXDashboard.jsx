...
import { Upload, Shield, AlertTriangle, CheckCircle, BarChart3, ArrowLeft, FileText, Loader2, Activity, Eye, Zap, Server, Users, Brain, Clock, Target, XCircle, Database, Gauge, TrendingUp, TrendingDown, Info } from 'lucide-react';
import { ResponsiveContainer, LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, PieChart, Pie, Cell } from 'recharts';
import { INWI_ICON } from '../ui/inwi';

const ComexDashboard = ({ returnToSelector }) => {
  const [selectedFile, setSelectedFile] = useState(null);
  const [reportType, setReportType] = useState('strategic_risk_posture');
  ...
  const REPORT_TYPES = [
    // Posture & Risk
    { value: 'strategic_risk_posture', label: 'Cyber Risk Posture', icon: Shield },
    { value: 'exposure_risk', label: 'Risk Exposure', icon: AlertTriangle },
    { value: 'threat_landscape', label: 'Threat Landscape', icon: Eye },

    // Financial
    { value: 'financial_impact_costs', label: 'Financial Impact - Incident Cost', icon: TrendingDown },
    { value: 'financial_impact_avoided', label: 'Financial Impact - Avoided Loss', icon: TrendingUp },

    // Program & Operations
    { value: 'security_program', label: 'Security Program', icon: Target },
    { value: 'incident_resolution', label: 'Incident Resolution', icon: Zap },

    // Governance & Strategy
    { value: 'regulatory_compliance', label: 'Regulatory Compliance', icon: CheckCircle },
    { value: 'strategic_alignment', label: 'Strategic Alignment', icon: Brain },
    { value: 'benchmark_sector', label: 'Sector Benchmark', icon: BarChart3 },
  ];
  const colors = {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/inwi3/upload', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  const renderChart = (type, data, title, reportType) => {
    ...
    const ChartContainer = ({ children }) => {
      ...
      // COMEX Chart descriptions for tooltips
      const chartTooltips = {
      ...
      const tooltipText = chartTooltips[title] || 'Analyse des données de sécurité pour la gouvernance COMEX.';
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 text-sm">COMEX data analysis</p>
            </div>
  ...
  return (
    <div className="min-h-full bg-level3-accent p-6 relative overflow-hidden">
      {/* Pattern Background */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3 bg-level3-muted p-3 rounded-md">
            <div className="p-3 bg-level3-primary/30 rounded-xl">
              <INWI_ICON color={'level3-primary'} />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-foreground">COMEX Dashboard</h1>
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="flex items-center justify-between p-3 rounded-lg border border-border">
                  ...
                  <div className="p-2 rounded-md bg-level3-primary/10"><FileText className="h-5 w-5 text-level3-primary" /></div>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg border border-border">
                  <div>
                  ...
                  <div className="p-2 rounded-md bg-level3-primary/10"><Upload className="h-5 w-5 text-level3-primary" /></div>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg border border-border">
                  <div>
                    <div className="text-xs text-muted-foreground">Rows Processed</div>
                    <div className="font-medium text-level3-primary">{analysisResult.rows_processed?.toLocaleString()}</div>
                  </div>
                  <div className="p-2 rounded-md bg-level3-primary/10"><Activity className="h-5 w-5 text-level3-primary" /></div>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg border border-border">
                  <div>
            ...
            <Card className="bg-card/80 backdrop-blur-sm border-border">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
                  <FileText className="size-5 text-level3-primary" />
              ...
              <CardContent className="space-y-6">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full p-3 rounded-lg border-2 text-left transition-all duration-200 ${reportType === type.value
                            ? 'border-level3-primary bg-level3-primary/10 text-level3-primary'
                            : 'border-border hover:border-level3-primary/50 hover:bg-primary/5'
                            }`}
                        >
                ...
                <div className="space-y-3">
                  ...
                  <div
                    className={`border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 ${dragActive
                      ? 'border-level3-primary bg-level3-primary/10'
                      : 'border-border hover:border-level3-primary/50'
                      } ${selectedFile ? 'border-green-500 bg-level3-muted' : ''
                      }`}
                    ...
                    {selectedFile ? (
                      <div className="space-y-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          className="mt-2 bg-level3-accent"
                ...
                <div className="flex gap-3 pt-4">
                  <Button
                    ...
                    className="flex-1 bg-level3-primary"
                  >
                    {isAnalyzing ? (
                      <>
                        ...
                        <Card key={idx} className="bg-card/80 border-border">
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="p-2 rounded-lg bg-level3-primary/10">
                                <Icon className="h-5 w-5 text-level3-primary" />
                              </div>
                            </div>
                          </CardContent>
                        </Card>
